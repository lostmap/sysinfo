...
write_mysql "Load Average" "$(echo $(nproc;uptime)| awk '{c=$1; f=substr($(NF-2),1,length($(NF-2))-1); s=substr($(NF-1),1,length($(NF-1))-1); t=substr($(NF),1,length($(NF))); gsub(",","",$(NF-2)); if(100*$(NF-2)/c >=100) print "<span_class=\"crit\">"f"</span>"" "s" "t" CPU(S): "c; else if(100*$(NF-2)/c>=70) print "<span_class=\"warn\">"f"</span>"" "s" "t" CPU(S): "c; else print f" "s" "t" CPU(S): "c;}'|awk '{f=$1; s=$2; t=$3; c=$5; gsub(",","",$2); if(100*$2/c>=100) print f" ""<span_class=\"crit\">"s"</span>"" "t" CPU(S): "c; else if(100*$2/c>=70) print f" ""<span_class=\"warn\">"s"</span>"" "t" CPU(S): "c; else print f" "s" "t" CPU(S): "c;}'| awk '{f=$1; s=$2; t=$3; c=$5; gsub(",","",$3); if(100*$3/c>=100) print f" "s" ""<span_class=\"crit\">"t"</span>"" CPU(S): "c; else if(100*$3/c>=70) print f" "s" ""<span_class=\"warn\">"t"</span>"" CPU(S): "c; else print f" "s" "t" CPU(S): "c;}'| awk '{gsub("_"," ",$0); print $0}');
