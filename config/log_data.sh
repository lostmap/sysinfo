...
CPU_num=$(nproc)
write_mysql "testavg" "$(uptime | awk '{f=substr($(NF-2),1,length($(NF-2))-1); s=substr($(NF-1),1,length($(NF-1))-1); t=substr($(NF),1,length($(NF))); gsub(",","",$(NF-2)); if(int($(NF-2))>=100) print "<span_class=\"crit\">"f"</span>"" "s" "t; else if(int($(NF-2))>=70) print "<span_class=\"warn\">"f"</span>"" "s" "t; else print f" "s" "t;}'|awk '{f=$1; s=$2; t=$3; gsub(",","",$2); if(int($2)>=100) print f" ""<span_class=\"crit\">"s"</span>"" "t; else if(int($2)>=70) print f" ""<span_class=\"warn\">"s"</span>"" "t; else print f" "s" "t;}'| awk '{f=$1; s=$2; t=$3; gsub(",","",$3); if(int($3)>=100) print f" "s" ""<span_class=\"crit\">"t"</span>"; else if(int($3)>=70) print f" "s" ""<span_class=\"warn\">"t"</span>"; else print f" "s" "t;}'| awk '{gsub("_"," ",$0); print $0}')";
