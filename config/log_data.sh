...
CPU_num=$(nproc)
write_mysql "test avg" "$(uptime | awk '{f=substr($(NF-2),1,length($(NF-2))-1); s=substr($(NF-1),1,length($(NF-1))-1); t=substr($(NF),1,length($(NF))); gsub(",","",$(NF-2)); if(int($(NF-2))>40) print "<span class=\"crit\">"f"</span>";else print f;}')";
