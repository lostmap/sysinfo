...
CPU_num=$(nproc)
write_mysql "testavg" "$(uptime | awk '{f=substr($(NF-2),1,length($(NF-2))-1); s=substr($(NF-1),1,length($(NF-1))-1); t=substr($(NF),1,length($(NF))); gsub(",","",$(NF-2)); if(int($(NF-2))>=100) print "<span class=\"crit\">"f"</span>"; else if(int($(NF-2))>=70) print "<span class=\"warn\">"f"</span>"; else print f;}')"
